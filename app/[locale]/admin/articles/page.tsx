...
const CategoryListPage = () => {
  ...
  const t = useTranslations("Admin_Article");
  ...
  const fetchCategories = async () => {
    try {
      ...
      alert(t("ErrorFetchingCategories"));
    }
  };
  ...
  const fetchSubCategories = async (categoryId: number) => {
    try {
      ...
      alert(t("ErrorFetchingSubcategories"));
    }
  };
  ...
  const handleDelete = async (journalId: number) => {
    ...
    try {
      ...
      console.error(t("ErrorDeletingJournal"), error);
      alert(t("ErrorDeletingJournal"));
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error(t("OperationError"), error);
  ...
  return (
    <div className="p-6 w-full space-y-6">
      <h1 className="text-3xl font-bold text-center mb-6">{t("Articles")}</h1>
                  ...
                  {expandedSubCategoryIds.includes(sub.id) && (
                    <div className="mt-4 overflow-auto">
                      {!sub.journals || sub.journals.length === 0 ? (
                        <p className="text-sm text-gray-500">{t("NoJournals")}</p>
                      ) : (
                        <div className="overflow-x-auto mb-16">
                          <p>
                            <span>
                              {sub.count}/{sub.requireCount}
                            </span>
                          </p>
                          <table className="min-w-full text-sm border border-gray-200">
                            <thead className="bg-gray-100">
                              <tr>
                                <th className="border p-2">{t("TitleAZ")}</th>
                                <th className="border p-2">{t("TitleEN")}</th>
                                <th className="border p-2">{t("TitleRU")}</th>
                                <th className="border p-2">{t("DescriptionAZ")}</th>
                                <th className="border p-2">{t("DescriptionEN")}</th>
                                <th className="border p-2">{t("DescriptionRU")}</th>
                                <th className="border p-2">{t("KeywordsAZ")}</th>
                                <th className="border p-2">{t("KeywordsEN")}</th>
                                <th className="border p-2">{t("KeywordsRU")}</th>
                                <th className="border p-2">{t("Status")}</th>
                                <th className="border p-2">{t("CreatedAt")}</th>
                                <th className="border p-2">{t("UpdatedAt")}</th>
                                <th className="border p-2">{t("File")}</th>
                                <th className="border p-2 breaks-word">{t("ApprovedFile")}</th>
                                <th className="border p-2">{t("Operations")}</th>
                              </tr>
                            </thead>
                            <tbody>
                              {sub.journals.map((j) => (
          ...
          {currentJournal && (
            <div className="grid gap-4">
              ...
              >
                ...
                <option value="beingwatched">{t("BeingWatched")}</option>
              ...
              {selectedStatus === "finished" ? (
                <>
                  <label className="block mb-1 font-medium">{t("TitleAZ")}</label>
                  <input
                    name="title_az"
                  ...
                  />
                  <label className="block mb-1 font-medium">{t("TitleEN")}</label>
                  <input
                    name="title_en"
                    ...
                    className="w-full border border-gray-300 rounded px-3 py-2"
                  />
                  <label className="block mb-1 font-medium">{t("TitleRU")}</label>
                  <input
                    name="title_ru"
                    ...
                    className="w-full border border-gray-300 rounded px-3 py-2"
                  />
                  <label className="block mb-1 font-medium">{t("DescriptionAZ")}</label>
                  <textarea
                    name="description_az"
                    ...
                    rows={2}
                  />
                  <label className="block mb-1 font-medium">{t("DescriptionEN")}</label>
                  <textarea
                    name="description_en"
                    ...
                    rows={2}
                  />
                  <label className="block mb-1 font-medium">{t("DescriptionRU")}</label>
                  <textarea
                    name="description_ru"
                    ...
                    rows={2}
                  />
                  <label className="block mb-1 font-medium">{t("SelectFile")}</label>
                  <input
                    type="file"
        ...
        <DialogContent className="w-full max-w-4xl h-[85vh] p-6 overflow-hidden bg-white">
          ...
          <div className="flex flex-col h-[70vh] overflow-y-auto mt-4 px-4 py-3 bg-gray-100 rounded-2xl border border-gray-300 shadow-inner space-y-5">
            {viewMessages.length === 0 ? (
              <p className="text-center text-lg text-gray-500">{t("NoMessages")}</p>
            ) : (
              viewMessages
                .sort(
                  (a, b) =>
