...
export default function LogsPage() {
    ...
    if (error) return <p className="text-red-600">Xəta baş verdi.</p>;
    if (isLoading || !data) return <p>Yüklənir...</p>;

    const totalPages = Math.ceil(data.total / limit);

    return (
        <section className="p-6">
            <h1 className="text-2xl font-bold mb-4">Server logları</h1>
                ...
                <table className="min-w-full text-sm">
                    <thead>
                        <tr className="bg-gray-100 dark:bg-gray-800 text-left">
                            ...
                            <th className="px-4 py-2">Metod</th>
                            <th className="px-4 py-2">İstifadəçiId</th>
                            <th className="px-4 py-2">İstifadəçi</th>
                            <th className="px-4 py-2">İstifadəçi Rolu</th>
                            <th className="px-4 py-2">URL</th>
                            <th className="px-4 py-2">Status</th>
                            <th className="px-4 py-2">Müddət (ms)</th>
